 ...
 * CAGRA_HNSW index-build pipeline. Accumulated stage totals are printed once at JVM shutdown.
 *
 * <p>Stages are tagged with the dominant resource they exercise ([GPU], [CPU], [DISK], [PCIe]).
 * Stages that move a known number of bytes also report effective throughput (MB/s), so DISK/PCIe
 * stages can be compared against sequential ceilings. Diagnostic-only scaffolding: disable with
 * {@code -Dcuvs.stageTimers=false} or remove wholesale.
 */
public final class StageTimers {
  ...
  static {
    if (ENABLED) {
      Runtime.getRuntime()
          .addShutdownHook(
              new Thread(() -> dump("cuvs-lucene writer stage timers"), "stage-timers-dump"));
    }
  }
